...
루트 `main.py`가 `backend.api.heartbeat`, `backend.api.hsportal` 라우터를 `/api` prefix로 등록합니다. `backend` 디렉터리 안에는 별도 FastAPI 진입점 파일을 두지 않습니다.
...
외부에서 사용할 수 있는 API는 저장된 프로그램 데이터를 읽는 기능과 백그라운드 크롤링 상태 확인 기능만 제공합니다. 크롤링 실행은 `HsportalCrawler.ensure_program_data()`, `crawl_full()`, `crawl_incremental()`, `refresh_existing_from_list()` 같은 내부 메소드로만 관리합니다.
